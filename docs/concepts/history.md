...
`defaultClassifiers` in `src/lib/history.ts` runs in this order. The first classifier that returns a non-null list wins.
...
`foldChains` links hops through `refs.inputs` and `refs.blockHashes`. A swap may link to a later swap or bridge. A send may link to a receive. Unenriched local-anchor hops may link to each other.
